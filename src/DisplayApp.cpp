...
void grl_boot_screen_widgets(void){
  ...
  lv_style_set_radius(&style_bullet, LV_RADIUS_CIRCLE);
    ...
    lv_obj_t * boot_main_panel = lv_tabview_create(lv_scr_act(), LV_DIR_TOP, tab_h);

  lv_obj_set_style_text_font(lv_scr_act(), font_normal, 0);

  if(disp_size == DISP_LARGE) {
      lv_obj_t * tab_btns = lv_tabview_get_tab_btns(boot_main_panel);
...
uint8_t decode_vbus_poll_data(uint8_t *aPollBuff, uint8_t index){
...
uint8_t decode_vconn_poll_data(uint8_t *aPollBuff, uint8_t index){
    
    uint8_t payload_length = aPollBuff[index++];
    uint16_t lvcon_vol = (aPollBuff[index++] | (aPollBuff[index++] << 8)); 
    uint16_t lvcon_cur = (aPollBuff[index++] | (aPollBuff[index++] << 8));
...
uint8_t decode_temp_poll_data(uint8_t *aPollBuff, uint8_t index){
    
    uint8_t payload_length = aPollBuff[index++];
    uint8_t temperature = (aPollBuff[index++] | (aPollBuff[index++] << 8)); 
...
void process_polling_cmd(uint8_t *aPollBuff){
    ...
    for(int i = 0; i< ldatafieldscnt; ++i){
        ...
        if(aPollBuff[lIndex++] == SOP)
            ; //Do nothing just check if this is Start delimiter
        else
            return;//else return because there is a data mismatch or we can loop here and check for Start delimiter

        switch((poll_data_fields_e)aPollBuff[lIndex++]){
            case VBUS_INFO:
                lIndex = decode_vbus_poll_data(aPollBuff,lIndex);
            break;
            case VCONN_INFO:
                lIndex = decode_vconn_poll_data(aPollBuff,lIndex);
            break;
            case TEMP_INFO:
                lIndex = decode_temp_poll_data(aPollBuff,lIndex);
            break;
        }
    }
}
...
void process_app_cmd(_rx_queue_struct_t rx_struct)
{
    ...
    switch( (cmd_type_e) rx_struct.rx_buffer[1] & 0x0F){
        ...
        break;
        case GET_CMD:
            process_get_cmd(rx_struct.rx_buffer);
        break;
        case PGM_CMD:
        break;
        default:
